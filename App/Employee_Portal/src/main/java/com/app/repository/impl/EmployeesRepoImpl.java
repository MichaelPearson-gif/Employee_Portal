...
public class EmployeesRepoImpl implements EmployeesRepo {
	...
	@Override
	public List<String> employeeList() throws BusinessException {
		...
		try(Session session = HibernateSessionFactory.getSession()){
			...
			// Query the DB for a list of employees ordered by manager and append it to the allEmployees list
			// An inner join will already sort the order by manager
			allEmployees = session.createQuery("SELECT CONCAT(e.first_name, e.last_name) FROM employees e INNER JOIN employee_manager em ON e.email = em.email").getResultList();
			...
			// Log the error message
			log.trace(e.getMessage());
			throw new BusinessException("Could not retrieve list of employees.");
